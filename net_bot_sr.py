...
#    text = clean(text)
    ...
    try:
        if superresolution:
            ...
            opath[-1] = 'sr_' + opath[-1]
            opath = '/'.join(opath)
            while not os.path.exists(opath):
                continue
            sleep(0.1)
            logging.info('Superresolution ended // ' + str(message.chat.id))
...
def styleCallback(call):
    ...
    try:
        if superresolution:
            logging.info('Superresolution started // ' + str(call.message.chat.id))
            channelsr.basic_publish(exchange='', routing_key='tosr', body = str(opath))
            opath = opath.split('/')
            opath[-1] = 'sr_' + opath[-1]
            opath = '/'.join(opath)
            while not os.path.exists(opath):
                continue
            sleep(0.1)
            logging.info('Superresolution ended // ' + str(call.message.chat.id))
