...
class GAN :
	...
	def build_model(self) :
		...
		t_real_image = tf.compat.v1.placeholder('float32', [self.options['batch_size'],
		                              img_size, img_size, 3],
		                              name = 'real_image')
		t_wrong_image = tf.compat.v1.placeholder('float32', [self.options['batch_size'],
                                       img_size, img_size, 3],
		                               name = 'wrong_image')

		t_real_caption = tf.compat.v1.placeholder('float32', [self.options['batch_size'],
				                        self.options['caption_vector_length']],
		                                name='real_captions')

		t_z = tf.compat.v1.placeholder('float32', [self.options['batch_size'],
		                      self.options['z_dim']], name='input_noise')

		t_real_classes = tf.compat.v1.placeholder('float32', [self.options['batch_size'],
										self.options['n_classes']],
		                                name='real_classes')

		t_wrong_classes = tf.compat.v1.placeholder('float32', [self.options['batch_size'],
										 self.options['n_classes']],
		                                 name='wrong_classes')

		t_training = tf.compat.v1.placeholder(tf.bool, name='training')
		...
		d_right_predictions = tf.equal(tf.argmax(input=disc_real_image_aux, axis=1),
		                               tf.argmax(input=t_real_classes, axis=1))
		d_right_accuracy = tf.reduce_mean(input_tensor=tf.cast(d_right_predictions,
		                                          tf.float32))

		d_wrong_predictions = tf.equal(tf.argmax(input=disc_wrong_image_aux, axis=1),
		                               tf.argmax(input=t_wrong_classes, axis=1))
		d_wrong_accuracy = tf.reduce_mean(input_tensor=tf.cast(d_wrong_predictions,
		                                          tf.float32))

		d_fake_predictions = tf.equal(tf.argmax(input=disc_fake_image_aux_logits, axis=1),
		                              tf.argmax(input=t_real_classes, axis=1))
		d_fake_accuracy = tf.reduce_mean(input_tensor=tf.cast(d_fake_predictions,
		                                         tf.float32))

		tf.compat.v1.get_variable_scope()._reuse = False

		print('Building the Loss Function')
		g_loss_1 = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(
	...
	def add_image_summary(self, name, var, max_outputs=1):
		with tf.compat.v1.name_scope('summaries'):
			tf.compat.v1.summary.image(name, var, max_outputs=max_outputs)
	...
	def attention(self, decoder_output, seq_outputs, output_size, time_steps,
		...
		with tf.compat.v1.variable_scope('g_a_attention'):
			ui = tf.transpose(a=ui, perm=[1, 0, 2])
			ai = tf.nn.softmax(ui,  axis=1)
			seq_outputs = tf.transpose(a=seq_outputs, perm=[1, 0, 2])
			d_dash = tf.reduce_sum(input_tensor=tf.mul(seq_outputs, ai), axis=1)
			return d_dash, ai
